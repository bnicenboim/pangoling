...
#'
#' @examples
preload_causal <- function(model = "gpt2",add_bos_token = NULL,...) {
  lang_model(model, task = "causal")
  tokenizer(model, add_bos_token = add_bos_token,...)
  invisible()
}
...
#' @param ... other arguments that control how the model from hugging face is accessed.
#'
#' @return
#'
#' @examples
#' @export
#' @export
get_causal_next_tokens_tbl <- function(context, model = "gpt2", add_bos_token = NULL, ...) {
  context_tensor <-
    tokenizer(model, add_bos_token = add_bos_token, ...)$encode(context, return_tensors = "pt")
