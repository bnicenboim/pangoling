...
test_that("can handle extra parameters", {
  skip_if_no_python_stuff()
  probs <- get_causal_log_prob(x = c("This", "is", "it"), add_special_tokens = TRUE)
  word_1_prob <- get_causal_next_tokens_tbl("<|endoftext|>")
  prob1 <- word_1_prob[token == "This"]$log_prob
  names(prob1) <- "This"
  expect_snapshot(probs)
  expect_equal(probs[1], prob1)

  probs_F <- get_causal_log_prob(x = c("This", "is", "it"), add_special_tokens = FALSE)
  expect_true(is.na(probs_F[1]))
  word_2_prob <- get_causal_next_tokens_tbl("This")
  prob2 <- word_2_prob[token == "Ġis"]$log_prob
  names(prob2) <- "is"
