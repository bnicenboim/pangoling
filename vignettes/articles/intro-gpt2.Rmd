...
Then let's examine which continuation GPT-2 predicts following a specific context. [Hugging Face](https://huggingface.co/) provide access to pre-trained models, including freely available versions of different sizes of  [GPT-2](https://huggingface.co/gpt2). The function `causal_next_tokens_pred_tbl()` will, by default, use the smallest version of [GPT-2](https://huggingface.co/gpt2), but this can be modified with the argument `model`.
...
Going back to the initial example, because `causal_next_tokens_pred_tbl` returns log (natural) probabilities, if we exponentiate them and we sum them, we should get 1:
...
This package would be most useful in the following situation. Given a (toy) dataset where sentences are organized with one word or short phrase in each row:
```{r, cache = TRUE}
sentences <- c(
...
Notice that the `by` is inside the `causal_words_pred()` function. It' also possible to use `by` in the mutate call, or `group_by()`, but it will be slower.
...
```{r}
df_token_lp <- causal_tokens_pred_tbl("The apple doesn't fall far from the tree.")
df_token_lp
