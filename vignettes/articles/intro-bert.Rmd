...
Whereas [the introductory vignette](articles/intro-gpt2) presents the most popular way to calculate word probabilities using GPT-like models, masked models present an alternative, especially, when we just care about the final word following a certain context.
...
```{r}
df_masks <- 
  masked_tokens_tbl("The apple doesn't fall far from the [MASK][MASK]")
...
The function `masked_lp()` will give us the log-probability of the target word (and will take care of summing the log-probabilities in case the target is composed by several tokens).
